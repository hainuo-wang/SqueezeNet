...
def main(args):
    ...
    # 如果存在预训练权重则载入
    model = SqueezeNet(in_channel=1, version=args.version, num_classes=args.num_classes).to(device)
    if args.weights != "":
    ...
    plot_accuracy(args.epochs, total_accuracy_train, total_accuracy_val, "SqueezeNet_CK+")
